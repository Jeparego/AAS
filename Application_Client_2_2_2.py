...
principalroot = Tk()
principalroot.title("Visor AAS Impresora 3D PRUSA")
...
def start_aas():
    ...
    subidenti.grid(row=0, column=0, sticky=W+E)
    ...
    identificacionaas = [
        ...
        "AñoDeConstruccion",
        "NombreDelFabricante",
        "DenominacionDelProducto"
        ]

    iaasclicked = StringVar()
    iaasclicked.set(identificacionaas[0])

    informacioni = LabelFrame(subident, text= 'Informacion Submodelo Identificación' , padx=margen, pady=margen, font=("Times",tsubtitle))
    informacioni.grid(row=1, column=0, sticky=W+E)


    opti = OptionMenu(informacioni, iaasclicked, *identificacionaas)
    opti.grid(row=0, column=0)

    optivalue = Label(informacioni, text ="0", padx=margen, pady=margen, relief=SUNKEN)
    optivalue.grid(row=0, column=2)
    ...
    subinfcontacti.grid(row=0, column=0, columnspan=3,sticky=W+E)
    ...
    clickedproveedor2=StringVar()
    clickedproveedor2.set(correo[0])
    ...
    informacionproveedorvalue.grid(row=0, column=2)
    ...
    salir = Button (root, text="Salir", padx=100, pady=10, bd=5, command=salir_app).grid(row=4,column=0, columnspan=3)
    ...
    async def main():    
        ...
        async with Client(url=url) as client:
            while flag:
                ## Código del Cliente Usuario
                ## Flag servirá como notificador del proceso de Tkinter
                ...
                iaasvar = await client.nodes.root.get_child(['0:Objects', '1:AASROOT', '1:Impresora3DPRUSA', '1:SubmodeloIdentificacion', '1:'+str(iaasclicked.get()), '2:Value'])
                optivalue['text']=str(await iaasvar.read_value())
                ...
                rutavar = await client.nodes.root.get_child(["0:Objects", "1:AASROOT", "1:Impresora3DPRUSA", "1:SubmodeloDocumentacion", "1:DocumentosOperacionales", "1:ArchivoDeManualDeOperacion", "2:Value"])
                manualoprutavalue['text'] = str(await rutavar.read_value())
                ...
                if (case != "CorreoElectronico" and case != "NumeroTelefonico"):
                    ...
                    if flag4:
                        informacionproveedor2["state"]=DISABLED
                if case == "CorreoElectronico" or case=="NumeroTelefonico":
                    if flag4:
                        informacionproveedor2["state"]=NORMAL
                    if case=="CorreoElectronico":
                        flag3=True
                        if flag2:
                            clickedproveedor2.set(correo[0])
                            flag2=False
                        informacionproveedor2 = OptionMenu(proveedor,clickedproveedor2, *correo)
                    elif case == "NumeroTelefonico":
                        flag2=True
                        if flag3:
                            clickedproveedor2.set(numero[0])
                            flag3=False
                        informacionproveedor2 = OptionMenu(proveedor,clickedproveedor2, *numero)
                    flag4=True
                    informacionproveedor2.grid(row=0, column=1)
                    varinfoporveedor = await client.nodes.root.get_child(['0:Objects', '1:AASROOT', '1:Impresora3DPRUSA', '1:SubmodeloInformacionDeContacto', '1:Proveedor1', '1:'+str(case)+'1', '1:'+str(clickedproveedor2.get())+'1', '2:Value'])
...
interface = LabelFrame(principalroot, text="Visor AAS Impresora 3D PRUSA", padx=10, pady=10, font=("Times", 15))
interface.grid(row=0, column=0, padx=20, pady=20)

interface2 = LabelFrame(principalroot, text="Ingrese Id del activo", padx=10, pady=10, font=("Times", 12))
interface2.grid(row=0, column=0, padx=20, pady=20)

e = Entry(interface2, width=45, borderwidth=5)
e.grid(row=0, column=0, padx=10, pady=10)

def verificar():
    s = str(e.get())
    e.delete(0, END)
    if s == "https://impresoras3dcolombia.co/IP3DPRUSA":
        start_aas()
        go["text"]="AAS iniciado"
        go["state"]=DISABLED
    else:
        messagebox.showerror("Id no encontrado", "Ingrese un Id válido por favor")
        
    

go = Button(interface2, text="Iniciar", padx = 10, pady=10, command=verificar)
